...
from config import bot, ADMIN_ID, DESTINATION
from const import USER_FORM_TEXT, START_MENU
from database.sql_commands import Database
from keyboards.inline_buttons import questionnaire_keyboard, like_dislike_keyboard, my_profile_keyboard
import random
import re
...
async def random_profiles_call(call: types.CallbackQuery):
    if call.message.caption.startswith("Ay yo"):
        pass
    else:
        try:
    ...
    if not profiles:
      await bot.send_message(
          chat_id=call.from_user.id,
          text="No mo fools,  you watched em all."
      )
    try:
...
async def like_detect_call(call:types.CallbackQuery):
    owner = re.sub("liked_profile", "", call.data)
    db = Database()
    try:
        db.sql_insert_like(
            owner=owner,
...
def register_profile_handlers(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(
        like_detect_call,
        lambda call: 'liked_profile' in call.data
    )
    dp.register_callback_query_handler(
        back_call,
